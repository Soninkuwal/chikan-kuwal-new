...
const initialMessages: Message[] = [
    { id: 1, sender: 'User', name: 'Ishaan', avatar: 'I', text: 'I have a question about my last deposit.', timestamp: '10:30 AM' },
    { id: 2, sender: 'Admin', name: 'Sonic Kuwal', avatar: 'A', text: 'Sure, what is the UTR number?', timestamp: '10:31 AM' },
];
...
export default function ChatPage() {
  ...
  useEffect(() => {
    const savedMessages = localStorage.getItem('userAdminChatMessages');
    if (savedMessages) {
        setMessages(JSON.parse(savedMessages));
    } else {
        localStorage.setItem('userAdminChatMessages', JSON.stringify(initialMessages));
    }

    const handleStorageChange = (event: StorageEvent) => {
        if (event.key === 'userAdminChatMessages') {
            const updatedMessages = localStorage.getItem('userAdminChatMessages');
            if (updatedMessages) {
  ...
  const sendMessage = (text: string, emoji?: string) => {
    if (text.trim() || attachment || emoji) {
        ...
        localStorage.setItem('userAdminChatMessages', JSON.stringify(updatedMessages));
        window.dispatchEvent(new StorageEvent('storage', { key: 'userAdminChatMessages' }));
        setNewMessage('');
        setAttachment(null);
        setReplyingTo(null);
    }
    ...
    <div className="flex-1 p-4 sm:p-8 pt-6 h-[calc(100vh-80px)] flex flex-col">
      ...
      <Card className="flex-1 flex flex-col">
        ...
        <CardContent className="flex-1 flex flex-col gap-4 overflow-y-auto p-4">
            {messages.map((msg) => (
                 <div key={msg.id} className={`group flex items-end gap-3 ${msg.sender === 'Admin' ? 'justify-end' : ''}`}>
                    {msg.sender === 'User' && (
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <div className={`p-3 rounded-lg max-w-[80%] sm:max-w-xs md:max-w-md ${msg.sender === 'Admin' ? 'bg-primary text-primary-foreground' : 'bg-secondary'}`}>
                            {msg.repliedTo && (
