...
export default function OwnerChatPage() {
  ...
  useEffect(() => {
    const ownerStr = localStorage.getItem('currentOwner');
    if(ownerStr){
        setCurrentUser(JSON.parse(ownerStr));
    }
  }, []);
  ...
  const sendMessage = (text: string, emoji?: string) => {
    ...
    const newMsg: any = {
        senderId: 'owner_id', // Static ID for owner
        role: 'Owner',
    ...
    <div className="flex-1 p-4 sm:p-8 pt-6 h-[calc(100vh-64px)] flex flex-col">
      ...
      <Card className="flex-1 flex flex-col">
        <CardHeader>
          ...
          <CardDescription>Chat with users and admins in real-time.</CardDescription>
        </CardHeader>
        <CardContent className="flex-1 flex flex-col gap-4 overflow-y-auto p-4">
            {messages.map((msg) => {
                const isYou = msg.senderId === 'owner_id';
                return (
                 <div key={msg.id} className={`group flex items-start gap-3 ${isYou ? 'justify-end' : 'justify-start'}`}>
                    ...
                    <div className={`flex flex-col gap-1 ${isYou ? 'items-end' : 'items-start'}`}>
                     ...
                     {isYou && (
                        <Avatar className="h-8 w-8">
                            <AvatarImage src={`https://placehold.co/40x40.png?text=O`} />
