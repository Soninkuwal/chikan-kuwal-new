...
const initialMessages: Message[] = [
    { id: 1, sender: 'Admin', name: 'Sonic Kuwal', avatar: 'A', text: 'There is a high volume of withdrawal requests.', timestamp: '10:30 AM' },
    { id: 2, sender: 'Owner', name: 'Owner', avatar: 'O', text: 'Understood. Please process them as quickly as possible.', timestamp: '10:31 AM' },
];
...
export default function OwnerChatPage() {
  ...
  useEffect(() => {
    const savedMessages = localStorage.getItem('ownerAdminChatMessages');
    if (savedMessages) {
        setMessages(JSON.parse(savedMessages));
    } else {
        localStorage.setItem('ownerAdminChatMessages', JSON.stringify(initialMessages));
    }

    const handleStorageChange = (event: StorageEvent) => {
        if(event.key === 'ownerAdminChatMessages') {
            const updatedMessages = localStorage.getItem('ownerAdminChatMessages');
            if (updatedMessages) {
  ...
  const sendMessage = (text: string, emoji?: string) => {
    if (text.trim() || attachment || emoji) {
        ...
        localStorage.setItem('ownerAdminChatMessages', JSON.stringify(updatedMessages));
        window.dispatchEvent(new StorageEvent('storage', { key: 'ownerAdminChatMessages' }));
        setNewMessage('');
        setAttachment(null);
        setReplyingTo(null);
    }
  ...
  return (
    <div className="flex-1 p-4 sm:p-8 pt-6 h-[calc(100vh-64px)] flex flex-col">
      <div className="flex items-center justify-between space-y-2 mb-4">
        <h2 className="text-3xl font-bold tracking-tight">Chat with Admins</h2>
      </div>
      <Card className="flex-1 flex flex-col">
        ...
        <CardContent className="flex-1 flex flex-col gap-4 overflow-y-auto p-4">
            {messages.map((msg) => (
                 <div key={msg.id} className={`group flex items-end gap-3 ${msg.sender === 'Owner' ? 'justify-end' : ''}`}>
                    {msg.sender === 'Admin' && (
                     ...
                     <div className="flex items-center gap-2">
                        ...
                        <div className={`p-3 rounded-lg max-w-xs md:max-w-md ${msg.sender === 'Owner' ? 'bg-primary text-primary-foreground' : 'bg-secondary'}`}>
                            {msg.repliedTo && (
