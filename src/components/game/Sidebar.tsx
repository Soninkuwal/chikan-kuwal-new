...
const adminChatInitialMessages = [
    { id: '1', senderId: 'admin_id', role: 'Admin' as const, name: 'Admin', avatar: 'A', text: 'Hello! How can I help you today?', timestamp: new Date().getTime() },
];
...
export function Sidebar({ 
  ...
  const handleChatClick = () => {
    onOpenChange(false);
    setTimeout(() => setActiveModal('chat_interface'), 200);
  }
  ...
  const getModalContent = () => {
    if (!user) return null;
     if (activeModal === 'chat_interface') {
        ...
        return <ChatInterface initialMessages={adminChatInitialMessages} storageKey="unifiedChat" currentUser={{name: user.name, avatar: avatarFallback, id: user.id}} />;
    }
  ...
  return (
    <>
      ...
      <InfoModal 
        ...
        title={
            activeModal === 'chat_interface' ? `Live Chat` :
