  ...
  useEffect(() => {
    if (isOpen) {
        ...
        if (savedSettings) {
            ...
            if (feeType === 'user') {
                 setFee(Number(settings.withdrawalFee) || 10);
                 setWithdrawalInfo(settings.withdrawalInfo || 'The initial demo amount is not withdrawable. Withdrawals are subject to admin approval. A {fee}% processing fee will be applied to your winnings. You can only make one withdrawal every 24 hours.');
            } else if (feeType === 'owner') {
                 setFee(Number(settings.ownerFee) || 2);
            } else {
                setFee(0);
            }
        } else {
             if (feeType === 'user') {
                setFee(10);
                setWithdrawalInfo('The initial demo amount is not withdrawable. Withdrawals are subject to admin approval. A 10% processing fee will be applied to your winnings. You can only make one withdrawal every 24 hours.');
             }
             else if (feeType === 'owner') setFee(2);
             else setFee(0);
        }
    }
  }, [isOpen, feeType]);
