...
export function WithdrawModal({ isOpen, onOpenChange, feeType = 'user', settings, user }: ModalProps) {
  ...
  const handleSubmit = async () => {
    ...
    if (feeType === 'user' && (!user || user.wallet < numericAmount)) {
    ...
    if(feeType === 'user') {
        ...
        const newWalletBalance = user.wallet - numericAmount;
        await update(ref(db, `users/${user.id}`), { wallet: newWalletBalance });
    }

    const newRequest = {
        userId: user?.id || 'admin/owner',
        user: user?.name || 'Admin/Owner',
  ...
  const isWithdrawalLocked = feeType === 'user' && kycStatus !== 'Verified';
                ...
                {isWithdrawalLocked ? (
                    ...
                    <>
                        {feeType === 'user' && kycStatus === 'Verified' && (
                             <Alert variant="default" className="border-green-500/50 text-green-500 [&>svg]:text-green-500">
